...
class Smoke3DEngine {
                    ...
                    for (l = 1; l <= N; l++) {
                        x[Smoke3DEngine.IX(N, i, j, l)] =
                            (x0[Smoke3DEngine.IX(N, i, j, l)] +
                                a * (x[Smoke3DEngine.IX(N, i - 1, j, l)] + x[Smoke3DEngine.IX(N, i + 1, j, l)] +
                                    x[Smoke3DEngine.IX(N, i, j - 1, l)] + x[Smoke3DEngine.IX(N, i, j + 1, l)] +
                                    x[Smoke3DEngine.IX(N, i, j, l - 1)] + x[Smoke3DEngine.IX(N, i, j, l + 1)])) / (1 + 6 * a);
                    }
                }
            }
    ...
    static advect(N, b, d, d0, u, v, w, dt) {
        ...
        for (i = 1; i <= N; i++) {
            for (j = 1; j <= N; j++) {
                for (l = 1; l <= N; l++) {
                    ...
                    w1 = z - l0;
                    w0 = 1 - w1;
                    d[Smoke3DEngine.IX(N, i, j, l)] =
                        s0 * (t0 * d0[Smoke3DEngine.IX(N, i0, j0, l0)] + t1 * d0[Smoke3DEngine.IX(N, i0, j1, l0)]) +
                            s1 * (t0 * d0[Smoke3DEngine.IX(N, i1, j0, l0)] + t1 * d0[Smoke3DEngine.IX(N, i1, j1, l0)]);
                }
            }
        }
        Smoke3DEngine.set_bnd(N, b, d);
    }
    static dens_step(N, x, x0, u, v, w, diff, dt) {
    ...
    static vel_step(N, u, v, w, u0, v0, w0, visc, dt) {
    ...
    static project(N, u, v, w, p, div) {
        let i, j, k, l;
        let h = 1 / N;
        for (i = 1; i <= N; i++) {
            for (j = 1; j <= N; j++) {
                for (l = 1; l <= N; l++) {
                    div[Smoke3DEngine.IX(N, i, j, l)] = -0.5 * h * (u[Smoke3DEngine.IX(N, i + 1, j, l)] - u[Smoke3DEngine.IX(N, i - 1, j, l)] +
        ...
        for (k = 0; k < 20; k++) {
            for (i = 1; i <= N; i++) {
                for (j = 1; j <= N; j++) {
                    for (l = 1; l <= N; l++) {
                        p[Smoke3DEngine.IX(N, i, j, l)] = (div[Smoke3DEngine.IX(N, i, j, l)] +
                            p[Smoke3DEngine.IX(N, i - 1, j, l)] +
                            p[Smoke3DEngine.IX(N, i + 1, j, l)] +
                            p[Smoke3DEngine.IX(N, i, j - 1, l)] +
                            p[Smoke3DEngine.IX(N, i, j + 1, l)] +
                            p[Smoke3DEngine.IX(N, i, j, l - 1)] +
                            p[Smoke3DEngine.IX(N, i, j, l + 1)]) / 6;
                    }
                }
            }
            Smoke3DEngine.set_bnd(N, 0, p);
        ...
        for (i = 1; i <= N; i++) {
            for (j = 1; j <= N; j++) {
                for (l = 1; l <= N; l++) {
                    u[Smoke3DEngine.IX(N, i, j, l)] -= 0.5 * (p[Smoke3DEngine.IX(N, i + 1, j, l)] - p[Smoke3DEngine.IX(N, i - 1, j, l)]) / h;
                    v[Smoke3DEngine.IX(N, i, j, l)] -= 0.5 * (p[Smoke3DEngine.IX(N, i, j + 1, l)] - p[Smoke3DEngine.IX(N, i, j - 1, l)]) / h;
                    w[Smoke3DEngine.IX(N, i, j, l)] -= 0.5 * (p[Smoke3DEngine.IX(N, i, j, l + 1)] - p[Smoke3DEngine.IX(N, i, j, l - 1)]) / h;
                }
            }
        }
